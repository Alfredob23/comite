...
class Facturar(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        # En esta parte valido le asigno valor a 'valor_pediente' y también analizo si el valor pagado es superior o inferior al valor total
        ...
        if not self.nFactura:
            ...
            max_num = 0
            for f in facturas:
                match = re.match(r'^FA(\d+)$', f.nFactura)
                if match:
                    num = int(match.group(1))
                    if num > max_num:
                        max_num = num
            self.nFactura = f"FA{max_num + 1}"
